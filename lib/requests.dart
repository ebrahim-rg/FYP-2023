...
class _RequestsState extends State<Requests> {
  ...
  Future<void> acceptgoingRequest(String userId, String day) async {
    setState(() {
  ...
  Future<void> acceptcomingRequest(String userId, String day) async {
    setState(() {
      _isLoading = true; // Show loading icon
    });
    ...
    try {
      final url = Uri.parse(
          'https://routify.cyclic.app/api/acceptcoming/${widget.userid}/$day');
      final response = await http.post(
        url,
        body: {'s_userid': userId}, // Include s_userid in the request body
      );
  ...
  Future<void> rejectRequest(String userId, String day) async {
    setState(() {
      _isLoading = true; // Show loading icon
    });
    ...
    try {
      final url = Uri.parse(
          'https://routify.cyclic.app/api/rejectreq/${widget.userid}/$day');
      final response = await http.post(
        url,
        body: {'s_userid': userId}, // Include s_userid in the request body
      );
                        ...
                        child: ExpansionTile(
                          ...
                          children: [
                            if (requestGoing.isNotEmpty)
                              buildRequestGoingList('Going', requestGoing, day),
                            if (requestComing.isNotEmpty)
                              buildRequestComingList(
                                  'Coming', requestComing, day),
                          ],
                        ),
                      );
                    },
  ...
  Widget buildRequestGoingList(
      String type, List<dynamic> requests, String day) {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            acceptgoingRequest(requestId, day);
                          },
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            acceptcomingRequest(requestId, day);
                          },
                          child: Icon(Icons.check, color: Colors.green),
                        ),
                        SizedBox(width: 16.0),
